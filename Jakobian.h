	...
	void add_matrixH(ElemUniv* elem, GlobalData* globalData, Factor* factor)
	{
		for (int i = 0; i < globalData->npc; i++)
		{
			...
			for (int k = 0; k < 4; k++)
			{
				...
				for (int j = 0; j < 4; j++)
				{
					temp.push_back(globalData->conductivity * detJ[i] * ((dNdX[i][k] * dNdX[i][j]) + dNdY[i][k] * dNdY[i][j]));
				}
		...
		for (int i = 0; i < globalData->npc; i++)
		{
			for (int j = 0; j < 4; j++)
			{
				for (int k = 0; k < 4; k++)
				{
					matrixH[j][k] += ilustrative_matrixH[i][j][k] * factor->factor[x] * factor->factor[y];
				}
			}
			if (p)
			{
	...
	void print_matirxH()
	{
		...
		for (int i = 0; i < ilustrative_matrixH.size(); i++)
		{
			...
			for (int j = 0; j < 4; j++)
			{
				for (int k = 0; k < 4; k++)
				{
					cout << setw(7) << ilustrative_matrixH[i][j][k] << " ";
				}
				cout << endl;
			}
		}
